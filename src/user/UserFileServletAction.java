...
public class UserFileServletAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(userRole == null) {
            ...
            } else if(action.equals("get_current_user_record_by_password")) {
                try {
                    String oldPassword = request.getParameter("old_password");
                    get_current_user_record_by_password(request, response, oldPassword);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
    ...
    protected void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String sql = "update " + tableName1 + " set user_id='" + userId  + "', title='" + title + "', content='" + content +  "', courseID='" +courseID  + "' where id='" + id + "'";
        conn.executeUpdate(sql);
        conn.close();
        postResult(request, response, action, null, "成功完成数据库交互", 3);
    }
    protected void get_current_user_record_by_password(HttpServletRequest request, HttpServletResponse response,String oldPassword) throws Exception {
        HttpSession session = request.getSession();
        ...
        String sql = "select * from " + tableName1 + " where userID='" + userId + "' and loginPassword='"+oldPassword+"'";
	...
	protected void changePassword(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String dbName = (String)session.getAttribute("unit_db_name");
        String tableName1 = this.tableName1;
        Integer tableID= Integer.valueOf(request.getParameter("id"));
        String newPassword= request.getParameter("newPassword");
        DbConn conn = new DbConn(dbName);
        String sql = "update "+tableName1+" set loginPassword='"+newPassword+"' where id="+tableID;
        System.out.println(sql);
