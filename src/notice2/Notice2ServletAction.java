...
//@WebServlet("/Notice2ServletAction")
@WebServlet("/notice2_servlet_action")
public class Notice2ServletAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(userRole == null) {
            ...
            } else if(action.equals("query_classID")) {
                try {
                    QueryClassID(request, response);
                } catch(Exception ex) {
                    ex.printStackTrace();
                }
	...
	protected void getBriefRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        System.out.println("getBriefRecord(userID):" + userID);
        DbConn conn = new DbConn(dbName);
        List aadata = new ArrayList();
     //   String sql = "select * from " + tableName1;id='" + id + "'"
	...
	protected void QueryClassID(HttpServletRequest request, HttpServletResponse response) throws Exception {
		HttpSession session = request.getSession();
        String action = request.getParameter("action");
        String dbName = (String)session.getAttribute("unit_db_name");
        String userID= (String) session.getAttribute("userID");
        ...
        DbConn conn = new DbConn(dbName);
        List aadata = new ArrayList();
        String sql = "select * from teacher_course where teacherID='"+userID+"'";
        ...
        conn.close();
        session.setAttribute("result_set", aadata);
        postResult(request, response, action, aadata, "交互成功！", 3);
	}
	...
	protected void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
     ...
     //   String sql = "update " + tableName1 + " set user_id='" + userId + "', sign_time='" + signTime + "', sign_ip='" + signIp + "',  lesson_id='" + lessonId + "' where id='" + id + "'";
        String sql = "update " + tableName1 + " set userID='" + userID + "', publisherName='"+publisherName+"', title='" + title + "', content='" + content +"',  courseID='" + courseId + "' where id='" + id + "'";
