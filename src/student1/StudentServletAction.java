...
public class StudentServletAction
  extends HttpServlet
{
    ...
    throws Exception
  {
    ...
    String sql = "select student_info.studentID,classID,studentName from student_course,student_info where courseID='"+courseID+"' and student_course.studentID=student_info.studentID;";
    System.out.println(sql);
    ResultSet rs = conn.executeQuery(sql);
    while (rs.next())
    {
      List tmp = new ArrayList();
      String studentID=rs.getString("student_info.studentID").trim();
      String studentName=rs.getString("studentName").trim();
      String classID=rs.getString("classID").trim();
      tmp.add(studentID);
      tmp.add(studentName);
      tmp.add(classID);
      aadata.add(tmp);
