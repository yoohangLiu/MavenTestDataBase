...
public class StudentScoreServletAction
  extends HttpServlet
{
  private String tableName1 = "student_grade";
  ...
  protected void getBriefRecord(HttpServletRequest request, HttpServletResponse response)
    throws Exception
  {
    HttpSession session = request.getSession();
    ...
    String sql = "select courseName,course_table.courseID,courseCredit,classPerformance,examGrades,examRatio " +
            " from course_table,student_grade"+
            " where student_grade.courseID=course_table.courseID and student_grade.studentID='"+userId+"'";
    ...
    while (rs.next())
    {
      ...
      tmp.add(rs.getString("course_table.courseID"));
      tmp.add(rs.getString("classPerformance"));
      tmp.add(rs.getString("examGrades"));
      tmp.add(rs.getString("examRatio"));
